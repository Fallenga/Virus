	...
	public void onBypassFeedback(Player player, String commandValue)
	{
		...
		if (clan != null)
		{
			...
			if (clanHall != null)
			{
				...
				{
					int _normalBuffTime = 7200;
					int _danceBuffTime = 7200;
					int _songBuffTime = 7200;
					
					final Set<Integer> _danceSkills = new HashSet<>();
					final Set<Integer> _songSkills = new HashSet<>();
					...
					else if ((cost == 0) || ((Config.BUFFER_ITEM_ID == 57) && player.reduceAdena("NPC Buffer", cost, this, true)) || ((Config.BUFFER_ITEM_ID != 57) && player.destroyItemByItemId("NPC Buffer", Config.BUFFER_ITEM_ID, cost, player, true)))
					{
						for (int skillId : SchemeBufferTable.getInstance().getScheme(player.getObjectId(), schemeName))
						{
							...
							int time = _normalBuffTime;
							
							if (_danceSkills.contains(skillId))
							{
								time = _danceBuffTime;
							}
							else if (_songSkills.contains(skillId))
							{
								time = _songBuffTime;
							}
							skill.applyEffects(this, player, false, time);
						}
					}
				}
				...
				{
					player.sendMessage("No eres Premium o no tienes clan con hall.");
					return;
				}
			}
		}
