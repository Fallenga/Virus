...
public class Teleporter extends Merchant
{
	...
	public void onBypassFeedback(Player player, String command)
	{
		...
		switch (st.nextToken())
		{
			...
			// Random Spawn Teleport's Comando GK
			case "random_farmzone1":
			{
				TeleportFarm1(player);
			}
			case "random_farmzone2":
			{
				TeleportFarm2(player);
			}
			case "random_farmzone3":
			{
				TeleportFarm3(player);
			}
			case "random_farmzone4":
			{
				TeleportFarm4(player);
			}
			case "random_farmzone5":
			{
				TeleportFarm5(player);
			}
			default:
			{
	...
	private static void TeleportFarm1(Player activeChar)
	{
		Properties properties = new Properties();
		try (FileInputStream fis = new FileInputStream("config/Custom/TeleportRandomFarmAreas.ini"))
		{
			properties.load(fis);
		}
		catch (IOException e)
		...
		String randomSpawns = properties.getProperty("RandomSpawns");
		if ((randomSpawns == null) || randomSpawns.isEmpty())
		{
			System.out.println("Configuração de Random Spawns não encontrada no arquivo .properties.");
			return;
		}
		
		String[] spawnPoints = randomSpawns.split(";");
		String selectedSpawn = spawnPoints[Rnd.get(spawnPoints.length)];
		String[] coords = selectedSpawn.split(",");
		
		int x = Integer.parseInt(coords[0]);
		int y = Integer.parseInt(coords[1]);
		int z = Integer.parseInt(coords[2]);
		
		// Sem delay, teleporta direto
		activeChar.teleToLocation(x, y, z, 0);
	}
	
	private static void TeleportFarm2(Player activeChar)
	{
		Properties properties = new Properties();
		try (FileInputStream fis = new FileInputStream("config/Custom/TeleportRandomFarmAreas.ini"))
		{
		...
		String randomSpawns = properties.getProperty("RandomSpawns2");
		if ((randomSpawns == null) || randomSpawns.isEmpty())
		{
			System.out.println("Configuração de Random Spawns não encontrada no arquivo .properties.");
			return;
	...
	private static void TeleportFarm3(Player activeChar)
	{
		Properties properties = new Properties();
		try (FileInputStream fis = new FileInputStream("config/Custom/TeleportRandomFarmAreas.ini"))
		{
		...
		String randomSpawns = properties.getProperty("RandomSpawns3");
		if ((randomSpawns == null) || randomSpawns.isEmpty())
		{
			System.out.println("Configuração de Random Spawns não encontrada no arquivo .properties.");
			return;
	...
	private static void TeleportFarm4(Player activeChar)
	{
		Properties properties = new Properties();
		try (FileInputStream fis = new FileInputStream("config/Custom/TeleportRandomFarmAreas.ini"))
		{
		...
		String randomSpawns = properties.getProperty("RandomSpawns4");
		if ((randomSpawns == null) || randomSpawns.isEmpty())
		{
			System.out.println("Configuração de Random Spawns não encontrada no arquivo .properties.");
			return;
	...
	private static void TeleportFarm5(Player activeChar)
	{
		Properties properties = new Properties();
		try (FileInputStream fis = new FileInputStream("config/Custom/TeleportRandomFarmAreas.ini"))
		{
		...
		String randomSpawns = properties.getProperty("RandomSpawns5");
		if ((randomSpawns == null) || randomSpawns.isEmpty())
		{
			System.out.println("Configuração de Random Spawns não encontrada no arquivo .properties.");
			return;
