...
public class RequestExEnchantSkill extends ClientPacket
{
		...
		switch (_type)
		{
			...
			case IMMORTAL:
			{
				if (Rnd.get(100) <= enchantSkillHolder.getChance(_type))
				{
					...
					addAndStoreEnchantedSkill(player, enchantedSkill);
	...
	/**
	 * Método auxiliar que actualiza el skill en memoria y fuerza su guardado en la base de datos para todos los registros del personaje (sin importar el class_index).
	 */
	private void addAndStoreEnchantedSkill(Player player, Skill enchantedSkill)
	{
		// Guarda para la clase activa actual en memoria y BD
		player.addSkill(enchantedSkill, true);
		
		// Actualiza la base de datos para todas las clases/subclases que ya posean este skill
		try (Connection con = DatabaseFactory.getConnection();
			PreparedStatement ps = con.prepareStatement("UPDATE character_skills SET skill_level = ?, skill_sub_level = ? WHERE charId = ? AND skill_id = ?"))
		{
			ps.setInt(1, enchantedSkill.getLevel());
			ps.setInt(2, enchantedSkill.getSubLevel());
			ps.setInt(3, player.getObjectId());
			ps.setInt(4, enchantedSkill.getId());
			ps.executeUpdate();
		}
		catch (Exception e)
		{
			LOGGER.warning("Could not update enchanted skill in database for all subclasses: " + e.getMessage());
		}
	}
}