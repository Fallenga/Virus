...
public class QueenAnt extends AbstractNpcAI
{
	...
	public String onKill(Npc npc, Player killer, boolean isSummon)
	{
		...
		if (npcId == QUEEN)
		{
			...
			long respawnTime = baseIntervalMillis + getRandom(-randomRangeMillis, randomRangeMillis);
			
			if (Config.STATIC_RESPAWN_ENABLED && (Config.QUEEN_ANT_RESPAWN_DATE != null))
			{
				final long staticRespawn = GrandBossManager.getInstance().getRespawnTime(Config.QUEEN_ANT_RESPAWN_DATE);
				if (staticRespawn > 0)
				{
					respawnTime = staticRespawn;
				}
			}
